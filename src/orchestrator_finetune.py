...
def run_fleet_finetuning(
    ...
    base_models = [
        ...
        and ("/base/" in m.replace("\\", "/") or m.replace("\\", "/").endswith("/base"))
    ]
    ...
    for idx, model_path in enumerate(base_models, 1):
        ...
        finetuned_dir = os.path.join("models", model_arch, "finetuned")
        os.makedirs(finetuned_dir, exist_ok=True)
        finetuned_model_path = os.path.join(
            finetuned_dir, f"orbita_predictor_{params_str}_finetuned.pth"
        )
        ...
        base_csv = base_csv_candidates[-1]
        for candidate in base_csv_candidates:
            if os.path.exists(candidate):
                base_csv = candidate
                break

        finetune_dir = os.path.join("data", "datasets", "finetuning", model_arch)
