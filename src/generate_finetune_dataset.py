...
def generate_active_learning_dataset(
    ...
    # =========================================================================
    print(f" [2/5] Generating unlabelled candidate pool"
          f" ({pool_size} samples)...")
    ...
    while valid_samples < pool_size:
        sma_cand = np.random.uniform(
            sma_bounds[0], sma_bounds[1], pool_size
        )
        ecc_cand = np.random.uniform(
            ecc_bounds[0], ecc_bounds[1], pool_size
        )
    ...
    for i in range(0, pool_size, batch_size):
        ...
        x_raw = np.column_stack([
            ...
            np.sin(l_in), np.cos(l_in), batch_tof
        ])
        ...
        _, std_pred = model.predict_with_uncertainty(
            x_tensor, num_samples=30
        )
    ...
    # =========================================================================
    print(f" [4/5] Selecting {hard_cases} Hard Cases and"
    ...
    print(f" [5/5] Calling Numerical Oracle to compute physical truth"
