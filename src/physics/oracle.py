...
# Pre-compute perturbation constants to save computation time inside ODE
# solvers
...
def eci_to_coe(mu, r_eci, v_eci):
    ...
    e_vec = (1.0 / mu) * (
        (v**2 - mu / r) * r_eci - np.dot(r_eci, v_eci) * v_eci
    )
...
def mee_to_coe(p, f, g, h, k, l_true):
    ...
    # Safety check: avoid division by zero if orbit is perfectly parabolic (e
    # = 1)
    sma = p / (1.0 - ecc**2)
...
def get_ground_truth(sma, ecc, inc, raan, aop, ta, tof):
    ...
    def orbit_dynamics(t, y):
        ...
        az_j3 = c_j3_dyn * (
            30.0 * z_pos**2 - 35.0 * z_pos**2 * z2_r2 - 3.0 * r**2
        )
