...
from physics.oracle import coe_to_eci, eci_to_coe
...
def get_keplerian(mu, r0, v0, dt):
    ...
    # Solve Kepler's Equation (M = E - e*sin(E)) for the final Eccentric
    # Anomaly
    E_curr = M_final
    tolerance = 1e-12
    max_iter = 100
    ...
    for _ in range(max_iter):
        # f(E) = E - e*sin(E) - M
        # f'(E) = 1 - e*cos(E)
        delta_E = (E_curr - ecc * np.sin(E_curr) - M_final) / (
            1.0 - ecc * np.cos(E_curr)
        )
        E_curr -= delta_E
