...
from physics.residuals import (
    compute_mee_residuals,
...
class TestComputeMeeResiduals:
    ...
    def test_output_shapes(self):
        ...
        mee_in, mee_res = compute_mee_residuals(
            **ISS_ORBIT, tof=300.0
        )

        assert mee_in.shape == (6,), (
            f"MEE inputs shape: {mee_in.shape}, expected (6,)"
        )
        assert mee_res.shape == (6,), (
    ...
    def test_true_longitude_residual_is_wrapped(self):
        ...
        _, mee_res = compute_mee_residuals(
            **ECCENTRIC_ORBIT, tof=900.0
        )

        L_residual = mee_res[5]
        assert -np.pi <= L_residual <= np.pi, (
...
class TestComputeAnalyticalState:
    ...
    def test_output_shapes(self):
        ...
        pos, vel, mee = compute_analytical_state(
            **ISS_ORBIT, dt=300.0
        )
    ...
    def test_position_magnitude_is_reasonable(self):
        ...
        """
        pos, _, _ = compute_analytical_state(
            **ISS_ORBIT, dt=300.0
        )
    ...
    def test_velocity_magnitude_is_reasonable(self):
        ...
        """
        _, vel, _ = compute_analytical_state(
            **ISS_ORBIT, dt=300.0
        )
    ...
    def test_mee_semilatus_rectum_positive(self):
        ...
        _, _, mee = compute_analytical_state(
            **ISS_ORBIT, dt=300.0
        )

        assert mee[0] > 0, (
            f"Semi-latus rectum p={mee[0]} is not positive"
        )
