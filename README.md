...
```mermaid
flowchart TB
    subgraph INPUTS["📡 Initial Orbital State (COE)"]
        COE["SMA, ECC, INC, RAAN, AOP, TA"]
    end

    subgraph GREYBOX["⚙️ Grey-Box Propagation Engine"]
        direction TB
        COE_MEE["COE → MEE Transformation"]

        subgraph ANALYTICAL["White-Box: Analytical Baseline"]
            KEPLER["Kepler Solver (Unperturbed)"]
            ESTHER["ESTHER CW Equations (J₂/J₃)"]
        end

        subgraph BLACKBOX["Black-Box: Neural Residual Correction"]
            ROUTER["3D MoE Router (SMA × ECC × INC)"]
            EXPERT["Specialized ResNet Expert"]
            MCDROPOUT["MC-Dropout Uncertainty σ"]
        end
    ...
    subgraph DECISION["🎯 Safety Decision Gate"]
        direction TB
        CHECK{"σ < 100 m?"}
        AI_OUT["✅ AI Prediction (Fast)"]
        GPS_RESET["🔄 GPS Reset: Oracle Call (Slow)"]
    end

    subgraph ORACLE["🔬 Numerical Oracle (Ground Truth)"]
        LSODA["LSODA Integration (Cowell)"]
    end

    subgraph LEARNING["📈 Continual Learning Pipeline"]
        direction TB
        HARD["Hard Case Mining (Uncertainty Sampling)"]
        REPLAY["Replay Buffer (Anti-Forgetting)"]
        FINETUNE["Fine-Tuning (μ Learning Rate)"]
    end

    COE --> GREYBOX
    ANALYTICAL --> |"pos + Δpos"| DECISION
    MCDROPOUT --> |"residual ± σ"| DECISION
    CHECK --> |"Yes"| AI_OUT
    CHECK --> |"No"| GPS_RESET
    GPS_RESET --> ORACLE
    ORACLE --> |"Exact State"| AI_OUT
    GPS_RESET -.-> LEARNING
    LEARNING -.-> |"Upgraded Weights"| EXPERT

    style INPUTS fill:#1a1a2e,stroke:#e94560,color:#ffffff
    style GREYBOX fill:#16213e,stroke:#0f3460,color:#ffffff
    style ANALYTICAL fill:#1a1a2e,stroke:#53a8b6,color:#ffffff
    style BLACKBOX fill:#1a1a2e,stroke:#e94560,color:#ffffff
    style DECISION fill:#0f3460,stroke:#e94560,color:#ffffff
    style ORACLE fill:#1a1a2e,stroke:#53a8b6,color:#ffffff
    style LEARNING fill:#1a1a2e,stroke:#ffc93c,color:#ffffff
    style AI_OUT fill:#2d6a4f,stroke:#40916c,color:#ffffff
    style GPS_RESET fill:#9b2226,stroke:#ae2012,color:#ffffff
```
...
* **Visualization & Analytics**: `Matplotlib` and `Seaborn` (CDF analysis and RIC error heatmaps).
* **Testing**: `pytest` (74 automated tests covering coordinate transforms, architectures, residuals, and configuration).

## 📂 Repository Structure
* `/data` - Dynamically generated MEE orbital datasets (`.csv`) and benchmark results.
...
* `/tests` - Automated test suite:
  ...
  * `test_architecture.py` - Forward pass shapes, MC-Dropout correctness, gradient flow for all architectures.
