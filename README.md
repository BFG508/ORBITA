...
* **Active Learning with MC-Dropout:** Implements an intelligent uncertainty-sampling mechanism via Monte Carlo Dropout. The Neural Network dynamically queries the computationally expensive Numerical Oracle only when its epistemic uncertainty drops below a strict safety threshold.
...
* `/data` - Dynamically generated orbital datasets for training expert models (ignored by git).
* `/models` - Saved PyTorch Neural Network weights (`.pth`) for the expert fleet.
* `/src` - Core framework source code:
  ...
  * `compare.py` - Evaluation script to compute the Euclidean residual error.
